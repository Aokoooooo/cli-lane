...
test("serialMode=global keeps all tasks in one FIFO lane", () => {
  ...
  scheduler.enqueue(task({ taskId: "task-1", serialMode: "global", mergeMode: "off", serialKey: "global" }));
  scheduler.enqueue(task({ taskId: "task-2", serialMode: "global", mergeMode: "off", serialKey: "global" }));
...
test("mergeMode=global forces tasks into the global lane", () => {
  ...
  scheduler.enqueue(task({ taskId: "task-1", serialMode: "by-cwd", mergeMode: "global", serialKey: "global" }));
  scheduler.enqueue(task({ taskId: "task-2", serialMode: "by-cwd", mergeMode: "global", serialKey: "global" }));

  expect(scheduler.nextRunnable("global")?.taskId).toBe("task-1");
  expect(scheduler.nextRunnable("/workspace")).toBeUndefined();
});
