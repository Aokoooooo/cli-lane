...
test('mergeMode=by-cwd merges queued task by cwd and argv', () => {
  ...
  const second = manager.createOrAttach(
    request({
      cwd: '/work/a',
      argv: ['bun', 'run', 'dev'],
      mergeMode: 'by-cwd',
...
test('mergeMode=off never merges identical requests', () => {
  ...
  const first = manager.createOrAttach(
    request({ cwd: '/work/a', argv: ['bun', 'run'], mergeMode: 'off' }),
  )
  const second = manager.createOrAttach(
    request({ cwd: '/work/a', argv: ['bun', 'run'], mergeMode: 'off' }),
  )
  ...
  expect(manager.listTasks().map((task) => task.taskId)).toEqual([
    first.taskId,
    second.taskId,
  ])
})
...
test('mergeMode=by-cwd does not merge when cwd differs', () => {
  ...
  const second = manager.createOrAttach(
    request({ cwd: '/work/b', argv: ['bun', 'run'], mergeMode: 'by-cwd' }),
  )

  expect(second.merged).toBe(false)
  expect(second.taskId).not.toBe(first.taskId)
  expect(manager.listTasks().map((task) => task.canonicalExecutionCwd)).toEqual(
    ['/work/a', '/work/b'],
  )
})
...
test('mergeMode=global merges by argv and keeps the first canonical execution cwd', () => {
  ...
  const second = manager.createOrAttach(
    request({ cwd: '/work/b', argv: ['bun', 'run'], mergeMode: 'global' }),
  )
...
test('detaching the last subscriber auto-cancels a queued task', () => {
  ...
  const created = manager.createOrAttach(
    request({ cwd: '/work/a', argv: ['bun', 'run'], mergeMode: 'by-cwd' }),
  )

  expect(manager.detachSubscriber(created.taskId, created.subscriberId)).toBe(
    true,
  )
  expect(manager.listTasks()).toEqual([
    {
...
test('listActiveTasks returns only queued and running tasks', () => {
  ...
  const queued = manager.createOrAttach(
    request({ cwd: '/work/queued', argv: ['bun', 'queued'], mergeMode: 'off' }),
  )
  const running = manager.createOrAttach(
    request({
      cwd: '/work/running',
      argv: ['bun', 'running'],
      mergeMode: 'off',
  ...
  const finished = manager.createOrAttach(
    request({
      cwd: '/work/finished',
      argv: ['bun', 'finished'],
      mergeMode: 'off',
  ...
  const canceled = manager.createOrAttach(
    request({
      cwd: '/work/canceled',
      argv: ['bun', 'canceled'],
      mergeMode: 'off',
...
test('recently canceled tasks also expire after the TTL', () => {
  ...
  const manual = manager.createOrAttach(
    request({
      cwd: '/work/manual',
      argv: ['bun', 'manual'],
      mergeMode: 'by-cwd',
  ...
  const auto = manager.createOrAttach(
    request({ cwd: '/work/auto', argv: ['bun', 'auto'], mergeMode: 'by-cwd' }),
  )
...
test('retained terminal snapshots stay stable when subscribers detach after finish or cancel', () => {
  ...
  const finishedFirst = manager.createOrAttach(
    request({
      cwd: '/work/finished-a',
      argv: ['bun', 'done'],
      mergeMode: 'global',
  ...
  const finishedSecond = manager.createOrAttach(
    request({
      cwd: '/work/finished-b',
      argv: ['bun', 'done'],
      mergeMode: 'global',
  ...
  const canceledFirst = manager.createOrAttach(
    request({
      cwd: '/work/canceled-a',
      argv: ['bun', 'stop'],
      mergeMode: 'global',
  ...
  const canceledSecond = manager.createOrAttach(
    request({
      cwd: '/work/canceled-b',
      argv: ['bun', 'stop'],
      mergeMode: 'global',
  ...
  expect(
    manager.detachSubscriber(finishedFirst.taskId, finishedSecond.subscriberId),
  ).toBe(true)
  expect(
    manager.detachSubscriber(canceledFirst.taskId, canceledSecond.subscriberId),
  ).toBe(true)
